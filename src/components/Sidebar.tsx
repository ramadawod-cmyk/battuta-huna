...
function BrandMark({ className, size = 32 }: { className?: string; size?: number }) {
  return (
    <svg className={className} width={size} height={size} viewBox="0 0 1024 1024" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
function DesktopSidebar() {
  ...
  return (
    <aside className="hidden md:flex w-[220px] lg:w-[260px] h-screen shrink-0 bg-white border-r border-text-primary/15 sticky top-0 flex-col">
      ...
      <nav className="px-[16px] lg:px-[20px] mt-[48px] lg:mt-[68px] flex flex-col gap-[8px]">
        {NAV_ITEMS.map((item) => {
...
function MobileHeader() {
  ...
  return (
    <>
      <header className="md:hidden fixed top-0 inset-x-0 z-40 h-[60px] bg-white border-b border-text-primary/15 flex items-center justify-between px-[20px]">
        <Link to="/explore" className="flex items-center gap-[8px]" onClick={() => setOpen(false)}>
      ...
      <div
        className={`md:hidden fixed inset-0 z-30 bg-white pt-[60px] transition-opacity duration-200 ${
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={`h-[52px] rounded-[12px] flex items-center gap-[14px] px-[16px] ${
                  active ? "bg-secondary-purple/10" : ""
                }`}
              >
                <item.Icon className={active ? "text-secondary-purple" : "text-text-secondary"} />
